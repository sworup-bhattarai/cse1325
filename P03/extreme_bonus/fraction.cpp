...
Fraction operator+(int a, const Fraction& rhs) 
{
		Fraction f{rhs._n * 1 + rhs._d * a, 1 * rhs._d };
		return f;
	
}
Fraction operator+(const Fraction& lhs, int a) 
{
		Fraction f{lhs._n * 1 + lhs._d * a, 1 * lhs._d };
		return f;
...
{
	int c = rhs._n * -1;
	Fraction f{a * rhs._d + 1 * c, 1 * rhs._d };
	//rhs(a/b) + (c/d) = (ad+bc)/bd;
	return f;
}
Fraction operator-(const Fraction& lhs , int a) 
{
	int c = a * -1;
	Fraction f{1 * lhs._n + lhs._d * c, lhs._d * 1 };
	//rhs(a/b) + (c/d) = (ad+bc)/bd;
	return f;
}
...
Fraction operator*(int a, const Fraction& rhs)
{
	Fraction f {a * rhs._n, 1 * rhs._d};
	return f;
}

Fraction operator*(const Fraction& lhs, int a)
{
	Fraction f {lhs._n * a, lhs._d * 1};
	return f;
}
...
Fraction operator/(const Fraction& lhs, int a)
{
	Fraction f {lhs._n * 1, lhs._d * a};
	return f;
}

Fraction operator/(int a, const Fraction& rhs)
{
	Fraction f {a * rhs._d, 1 * rhs._n};
	return f;
}
